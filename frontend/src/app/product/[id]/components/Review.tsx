...
import { Button, Form, Input, message, Modal, Progress, Rate } from 'antd';
...
const Reviews: React.FC<any> = ({ productId }) => {
  ...
  return (
    <div className="mt-8">
      ...
      {/* Tính toán và hiển thị phần trăm đánh giá */}
        ...
        <div className="space-y-4">
          {reviewsByProductId?.map((review: any) => (
            <div key={review._id} className="flex items-start">
              <img src="https://readymadeui.com/team-2.webp" alt={`Reviewer ${review.user_id.email}`} className="w-12 h-12 rounded-full border-2 border-white" />
              <div className="ml-3 flex-grow">
                ...
                <div className="flex space-x-1 mt-1">
                  {[...Array(5)].map((_, index) => (
                    <svg key={index} className={`w-4 ${index < review.rating ? 'fill-orange-400' : 'fill-[#CED5D8]'}`} viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M7 0L9.4687 3.60213L13.6574 4.83688L10.9944 8.29787L11.1145 12.6631L7 11.2L2.8855 12.6631L3.00556 8.29787L0.342604 4.83688L4.5313 3.60213L7 0Z" />
                    </svg>
                  ))}
                </div>
      ...
      {editingReview && (
        <Modal title="Edit Review" visible={isEditModalVisible} onCancel={handleEditCancel} footer={null} width={800}>
          <Form initialValues={{ rating: editingReview?.rating, review_text: editingReview?.review_text }} onFinish={handleEdit}>
            <Form.Item name="rating" rules={[{ required: true, message: 'Please rate the product!' }]}>
              <Rate />
            </Form.Item>
            <Form.Item name="review_text" rules={[{ required: true, message: 'Please write a review!' }]}>
              <Input.TextArea />
            </Form.Item>
